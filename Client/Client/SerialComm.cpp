...
bool SerialComm::Write(char* line, unsigned long lineSize, bool* isLineUsed) //в PacketMaker isLineUsed используется и для указание на то, заполнена ли строка, разделить
{
	if(isLineUsed == NULL)
	{
	...
	//линию не копирует, а может стоило бы?
	...
	Sleep(0); //дать возможность порожденному потоку залочить мьютекс
...
{
	_writeMutex.lock(); //какая гарантия, что первым запустится и залочит мьютекс именно этот поток
...
void SerialComm::_readThread()
{
	...
	while(_isReadingContinued)
	{
		...
		if(bytesRead > 0)
		{
			...
			{
				//обработчики должны скопировать буффер, чтобы использовать
